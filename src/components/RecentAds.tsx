    ...
    <section className="py-8 bg-gray-100">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link to="/c/todas" className="text-sm text-viva-green hover:underline">
            Ver todos →
          </Link>
        </div>
        ...
        {isLoading ? (
             <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                 {[...Array(4)].map((_, i) => (
                     <div key={i} className="bg-white rounded h-64 animate-pulse"></div>
                 ))}
             </div>
        ) : ads.length === 0 ? (
            ...
            {ads.map((ad) => (
                ...
                key={ad.id}
                to={`/anuncio/${ad.id}`}
                className="bg-white border border-gray-200 rounded overflow-hidden hover:shadow-md transition-shadow cursor-pointer block"
                >
